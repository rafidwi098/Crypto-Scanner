...
def main():  
    ...
    with st.sidebar:  
        ...
        st.markdown("""  
        <div class="info-box">  
        <b>📡 Data Source: CoinGecko API</b><br>  
        ✅ No Geographic Restrictions<br>  
        ✅ Free API (No Key Required)<br>  
        ℹ️ Daily Data Only<br>  
        ℹ️ ~300+ Cryptocurrencies  
        </div>  
        """, unsafe_allow_html=True)  
        ...
        lookback = st.slider(  
            ...
            value=100,  
            step=10,  
            help="Jumlah hari untuk analisa"  
        )  
    ...
    if st.session_state.get('should_fetch', False):  
        ...
        # Info sections  
        with st.expander("ℹ️ Tentang CoinGecko API"):  
            st.markdown("""  
            ### CoinGecko Free API  
            
            **Kelebihan:**  
            - ✅ Gratis tanpa API key  
            - ✅ Tidak ada geographic restriction (cocok untuk Indonesia)  
            - ✅ Data real-time dan akurat  
            - ✅ Support 300+ cryptocurrencies  
            
            **Keterbatasan:**  
            - ⚠️ Daily data saja (bukan 15m, 1h, 4h)  
            - ⚠️ ~10-50 calls per minute limit  
            - ⚠️ OHLC di-approximate dari close price  
            ...
            **Supported Pairs:**  
            ...
            - Ripple (XRP/USDT)  
            - Cardano (ADA/USDT)  
            - BNB, DOGE, MATIC, AVAX, DOT, dan lainnya  
            
            **Alternatif:**  
            - Untuk real-time data: Gunakan VPN + Binance/Bybit  
            - Untuk analisa jangka panjang: CoinGecko sudah cukup baik  
            """)  
        
        with st.expander("📊 SMC Indicators - Penjelasan"):  
            st.markdown("""  
            ### Smart Money Concept Indicators:  
            
            **1️⃣ Break of Structure (BOS)**  
            - **Bullish BOS**: Price tutup di atas recent swing high  
            - **Bearish BOS**: Price tutup di bawah recent swing low  
            - **Signal**: Trend continuation  
            - **Contoh**: BTC tembus resistance = bullish signal  
            
            **2️⃣ Change of Character (CHOCH)**  
            - **Uptrend**: HH (Higher High) + HL (Higher Low)  
            - **Downtrend**: LL (Lower Low) + LH (Lower High)  
            - **Reversal**: Perubahan dari satu pola ke pola lain  
            - **Signal**: Major trend change  
            
            **3️⃣ Order Blocks (OB)**  
            - **Accumulation**: High volume + tight range + bullish candle  
            - **Distribution**: High volume + tight range + bearish candle  
            - **Meaning**: Area di mana smart money buy/sell  
            
            **4️⃣ Fair Value Gaps (FVG)**  
            - **Bullish FVG**: Gap up antara candles  
            - **Bearish FVG**: Gap down antara candles  
            - **Probability**: ~75% kemungkinan akan diisi  
            - **Use**: Support/Resistance levels  
            
            **5️⃣ Liquidity Sweeps**  
            - **Stop Hunt**: Price menyentuh level lalu reverse  
            - **Bearish Sweep**: Terobos support → bullish signal  
            - **Bullish Sweep**: Terobos resistance → bearish signal  
            - **Signal Strength**: Tergantung recovery/pullback %  
            """)  
        
        with st.expander("🎯 Trading Strategy Tips"):  
            st.markdown("""  
            ### Entry Strategy:  
            1. **Tunggu BOS** dengan volume tinggi  
            2. **Konfirmasi dengan Order Block** di area support/resistance  
            3. **Enter saat liquidity sweep** recovery/pullback  
            4. **Target FVG zones** sebagai take profit  
            
            ### Risk Management:  
            - 🛑 Risk 1-2% per trade  
            - 🛑 Stop loss di swing levels  
            - 🎯 Minimum risk-reward 1.5:1  
            - 🎯 Tidak boleh add ke losing position  
            
            ### Best Practices:  
            - ✅ Gunakan multiple timeframes  
            - ✅ Konfirmasi signals across timeframe  
            - ✅ Backtest strategy sebelum real trading  
            - ✅ Monitor volume setiap setup  
            - ✅ Manajemen emosi adalah kunci  
            
            ### Disclaimer:  
            ⚠️ **Ini hanya untuk educational purposes**  
        ...
        with st.expander("🔧 Technical Details"):  
            st.markdown("""  
            ### Data Processing:  
            - CoinGecko menyediakan close price  
            - High/Low di-approximate: ±2% dari close  
            - Open = average 2 consecutive closes  
            
            ### Calculation:  
            - Swing High/Low: Local extrema dalam 5-candle window  
            - BOS: Close breach + volume confirmation  
            - CHOCH: Trend reversal detection  
            - Order Blocks: Volume + range analysis  
            - FVG: Gap detection between candles  
            - Sweeps: Price extreme
